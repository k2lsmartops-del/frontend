...
export default function ValidationCoordinateurPage() {
  ...
  const loadSubmissions = useCallback(async () => {
    try {
      const pendingRes = await api.get('/submissions', { params: { status: 'SUBMITTED', limit: 50 } });
      setSubmissions(pendingRes.data.data || []);
    } catch {
  ...
  return (
    <div>
      <h2 className="mb-4 font-head text-lg font-semibold text-k2l-gray-800">Validations en attente</h2>
      {total === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center text-k2l-gray-400">
          ...
          <p className="font-medium">Aucune soumission en attente de validation</p>
          <p className="text-sm">Toutes les fiches soumises ont été traitées</p>
        </div>
      ) : (
        <>
          {/* Header */}
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="font-head text-lg font-semibold text-k2l-gray-800">Validation</h2>
              <p className="text-sm text-k2l-gray-400">
                {total} fiche(s) en attente de validation
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl bg-white border border-k2l-gray-200 p-4">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="rounded-full bg-k2l-amber/10 text-k2l-amber px-2 py-0.5 text-xs font-medium">
                  En attente
                </span>
              </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="border-t border-k2l-gray-100 pt-3">
                  <div className="text-k2l-gray-400 text-xs flex items-center gap-1"><RiCalendarLine /> Date de soumission</div>
                  <div className="text-xs text-k2l-gray-600">
                    {current.submittedAt ? new Date(current.submittedAt).toLocaleString('fr-FR') : '—'}
                  </div>
                </div>
              </div>
            </div>
